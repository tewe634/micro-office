...
SELECT 'ADMIN', '/admin/portal-templates'
WHERE NOT EXISTS (
    SELECT 1 FROM role_menu_permission WHERE role = 'ADMIN' AND menu_key = '/admin/portal-templates'
);
...
SELECT su.id, '/admin/portal-templates'
FROM sys_user su
WHERE su.role = 'ADMIN'
  ...
  AND NOT EXISTS (
      ...
      WHERE ump.user_id = su.id
        AND ump.menu_key = '/admin/portal-templates'
  );
