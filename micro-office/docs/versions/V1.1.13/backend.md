...
- `GET /api/admin/workflow-templates/packages/{id}` 可以附带返回 `nodeGraph` / `nodeGraphDetail`
- 但它不是前端工作流画布的唯一主读取契约
- `PUT /api/admin/workflow-templates/packages/{id}` 不承担 `nodeGraph` 主保存职责
