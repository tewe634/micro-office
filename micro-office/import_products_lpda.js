#!/usr/bin/env node
// 按“三级类别名称”从 LPDA Excel 导入产品，并归档到指定目录结构
// 用法:
// node import_products_lpda.js <xlsx路径> <三级类别名称> <产品线> <一级分类> [二级分类] [系列展示口径]
// 例子:
// node import_products_lpda.js "/mnt/d/BaiduNetdiskDownload/低功率传动与自动化产品 LPDA.xlsx" "ACS880-04(R10-R11)" "ABB" "DP" "ACS880-01/04(R1-R11)"
...
function usage() {
  console.error('用法: node import_products_lpda.js <xlsx路径> <三级类别名称> <产品线> <一级分类> [二级分类] [系列展示口径]');
  process.exit(1);
}
...
const categoryLevel3 = normalizeArg(process.argv[3], { required: true });
const productLine = normalizeArg(process.argv[4], { required: true });
const structureLevel1 = normalizeArg(process.argv[5], { required: true });
const structureLevel2 = normalizeArg(process.argv[6]);
const seriesDisplayName = normalizeArg(process.argv[7]);
...
async function main() {
  ...
  const matchedRows = rows.filter((row) => String(row['三级类别名称'] || '').trim() === categoryLevel3);
  if (!matchedRows.length) {
    console.log(`未匹配到三级类别名称: ${categoryLevel3}`);
    return;
  }
  ...
  console.log(`  匹配三级类别: ${categoryLevel3}`);
