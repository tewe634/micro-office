...
async function main() {
  ...
  const parentCustomer = await api('POST', '/objects', {
    ...
    body: {
      ...
      industry: '制造业',
      customerRole: '原始设备制造商',
      customerScale: '普通客户-N',
      customerHealth: '稳定',
      ownerId: adminUserId,
    },
  });
  ...
  assert(fetchedObject?.customerRole === '系统集成商', 'object customerRole update not visible', fetchedObject);
  assert(fetchedObject?.customerScale === '大客户-M', 'object customerScale update not visible', fetchedObject);
  ...
  assert(objectPortal?.header?.customerScale === '大客户-M', 'object portal header missing customerScale', objectPortal);
