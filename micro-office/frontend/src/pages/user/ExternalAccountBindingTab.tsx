...
import {
  ...
  type UserExternalAccountProvider,
  type UserExternalAccountStatus,
} from '../../api';
...
export default function ExternalAccountBindingTab() {
  ...
  const openEdit = async (record: BindingRecord) => {
    ...
    try {
      ...
      const matched = accounts.find((item: any) => {
    ...
    <>
      ...
      >
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          ...
          <Form form={form} layout="vertical" disabled={!canManagePersonnel}>
            ...
            {selectedBindingUserId ? (
              <div style={{ marginBottom: 12, padding: 12, borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                {(() => {
                  const context = resolveUserContext(asText(selectedBindingUserId));
                  return (
